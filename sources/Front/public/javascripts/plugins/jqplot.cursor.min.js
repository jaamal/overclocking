...
(function (j) {
    ...
    j.jqplot.Cursor.postDraw = function () {
        ...
        this.eventCanvas._elem.before(w.zoomCanvas.createElement(this._gridPadding, "jqplot-zoom-canvas", this._plotDimensions));
        w.zoomCanvas.setContext();
        w._tooltipElem = j('<div class="jqplot-cursor-tooltip" style="position:absolute;display:none"></div>');
        w.zoomCanvas._elem.before(w._tooltipElem);
        if (w.showVerticalLine || w.showHorizontalLine) {
            w.cursorCanvas = new j.jqplot.GenericCanvas();
            this.eventCanvas._elem.before(w.cursorCanvas.createElement(this._gridPadding, "jqplot-cursor-canvas", this._plotDimensions));
            w.cursorCanvas.setContext()
        }
        if (w.showTooltipUnitPosition) {
            if (w.tooltipAxisGroups.length === 0) {
                var t = this.series;
                var u;
                var q = [];
                for (var r = 0; r < t.length; r++) {
                    u = t[r];
                    var v = u.xaxis + "," + u.yaxis;
                    if (j.inArray(v, q) == -1) {
                        q.push(v)
                    }
                }
                for (var r = 0; r < q.length; r++) {
                    w.tooltipAxisGroups.push(q[r].split(","))
                }
            }
        }
    };
    ...
    j.jqplot.Cursor.prototype.resetZoom = function (u, v) {
        ...
        if (!u.plugins.cursor.zoomProxy && v._zoom.isZoomed) {
            for (var r in t) {
                ...
                t[r].min = s[r].min;
                t[r].max = s[r].max;
                t[r].numberTicks = s[r].numberTicks;
                t[r].tickInterval = s[r].tickInterval;
                t[r].daTickInterval = s[r].daTickInterval
            }
            u.redraw();
    ...
    function i(s, r, v, u, t) {
        var w = t.plugins.cursor;
        var q = w.zoomCanvas._ctx;
        if (w.show) {
            if (w.showTooltip) {
                e(r, v, t);
                if (w.followMouse) {
    ...
    j.jqplot.CursorLegendRenderer.prototype.draw = function () {
        if (this.show) {
            var v = this._series, z;
            this._elem = j('<table class="jqplot-legend jqplot-cursor-legend" style="position:absolute"></table>');
            var q = false;
            for (var w = 0; w < v.length; w++) {
                z = v[w];
                if (z.show && z.showLabel) {
                    var u = j.jqplot.sprintf(this.formatString, z.label.toString());
                    if (u) {
                        var r = z.color;
                        if (z._stack && !z.fill) {
                            r = ""
                    ...
                    for (var t = 0; t < j.jqplot.addLegendRowHooks.length; t++) {
                        var x = j.jqplot.addLegendRowHooks[t].call(this, z);
                        if (x) {
                            y.call(this, x.label, x.color, q);
                            q = true
                        }
                    }
                }
        ...
        function y(C, B, E, s) {
            var A = (E) ? this.rowSpacing : "0";
            var D = j('<tr class="jqplot-legend jqplot-cursor-legend"></tr>').appendTo(this._elem);
            D.data("seriesIndex", s);
            j('<td class="jqplot-legend jqplot-cursor-legend-swatch" style="padding-top:' + A + ';"><div style="border:1px solid #cccccc;padding:0.2em;"><div class="jqplot-cursor-legend-swatch" style="background-color:' + B + ';"></div></div></td>').appendTo(D);
            var F = j('<td class="jqplot-legend jqplot-cursor-legend-label" style="vertical-align:middle;padding-top:' + A + ';"></td>');
            F.appendTo(D);
            F.data("seriesIndex", s);
            if (this.escapeHtml) {
                F.text(C)
            } else {
                F.html(C)
