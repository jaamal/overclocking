...
public class ConcurrentFcmpTest extends IntegrationTestBase {
    private SLPBuildHelper slpBuilder;
    ...
    public void setUp() {
        super.setUp();
        slpBuilder = container.create(SLPBuildHelper.class);
        ...
        IPatternMatchingConfig config = new IPatternMatchingConfig() {
            ...
            public LocalSearchStrategy getLocalSearchStrategy() {
                return LocalSearchStrategy.RecursiveWithContains;
            }
        };
    ...
    private void doTest(String text) {
        ...
        Product[] textSlp = slpBuilder.buildSlp(container.get(ISettings.class), text);
    ...
    private void doTest(Product[] textSlp, String text, int patternSize) {
        ...
        Product[] patternSlp = slpBuilder.buildSlp(container.get(ISettings.class), pattern);
        System.out.println("PatternSlpSize: " + patternSlp.length);
        long total = 0;
        int count = 3;
        for (int i = 0; i < count; ++i) {
            long tableBuildingBegin = System.currentTimeMillis();
            IAPTable table = tableBuilder.build(patternSlp, textSlp);
            long tableBuildingEnd = System.currentTimeMillis();
            total += (tableBuildingEnd - tableBuildingBegin);
        }
        System.out.println("BuildTime: " + (total / count) + " ms");
    }
